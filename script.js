...
(() => {
  ...
  declineBtn.addEventListener("click", () => {
    popup.classList.remove("active");
    window.location =
      "https://www.ni.com/en/support/downloads/drivers/download.frc-game-tools.html?srsltid=AfmBOooU5xxVVEt5dXOBapft2WQhnlLogObFARDtCx49sZGfLrO_XKA2";
  ...
  function openWS() {
    ...
    ws.onopen = () => {
      ...
      if (team > 0) {
        ws.send(JSON.stringify({ type: MessageType.SET_TEAM, team }));
      }
      ws.send(
        JSON.stringify({ type: MessageType.SET_MODE, mode: currentMode }),
      );
    };
  ...
  function handleStatus(status) {
    ...
    const pctLoss = Math.max(
      0,
      Math.min(100, Math.round((1 - packetWindow.length / expected) * 100)),
    );
  ...
  enableBtn.addEventListener("click", () => {
    ...
    ws.send(JSON.stringify({ type: MessageType.ENABLE }));
    log("Enable sent.");
  });
  (() => {
    ...
    downloadLogBtn.addEventListener("click", () => {
      let logText = Array.from(logStream.children)
        .map((e) => e.textContent)
        .join("\n");
  ...
  disableBtn.addEventListener("click", () => {
    ...
    ws.send(JSON.stringify({ type: MessageType.DISABLE }));
    log("Disable sent.");
  });
  ...
  estopBtn.addEventListener("click", () => {
    ...
    ws.send(JSON.stringify({ type: MessageType.ESTOP }));
    log("E-STOP sent!");
  });
  ...
  modeButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      setModeActive(mode);
      if (ws && ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify({ type: MessageType.SET_MODE, mode }));
        log(`Mode → ${mode}`);
      } else {
  ...
  // Auto-init
  loadPersistedTeam();
  setModeActive(RobotModeString.TELEOP);
  setConnState(connectionState.DISCONNECTED, "Disconnected");
  ...
  setInterval(() => {
    if (ws && ws.readyState === WebSocket.OPEN) {
      ...
      ws.send(
        JSON.stringify({ type: MessageType.PING, clientTs: lastPingSent }),
      );
    }
  }, 1000);
})();
