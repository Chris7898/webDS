...
class DriverStation {
  ...
  saveTeamToConfig() {
    fs.writeFileSync(this.configPath, JSON.stringify({ teamNumber: this.team }));
  }
...
setTeam(team) {
  ...
  sendDSPacket() {
    ...
    this.socket.send(buf, 0, buf.length, this.robotPort, this.robotIP, (err) => {
  ...
  modeToNumber(mode) {
    switch(mode) {
      case 'teleop': return 0;
      case 'autonomous': return 1;
      case 'test': return 2;
      default: return 0;
    }
  }
